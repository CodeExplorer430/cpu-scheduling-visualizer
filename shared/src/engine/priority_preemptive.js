...
exports.runPriorityPreemptive = runPriorityPreemptive;
var utils_js_1 = require('./utils.js');
function runPriorityPreemptive(inputProcesses, options) {
  var _a, _b;
  if (options === void 0) {
    options = {};
  }
  var _c = options.contextSwitchOverhead,
    contextSwitchOverhead = _c === void 0 ? 0 : _c,
    _d = options.enableLogging,
    enableLogging = _d === void 0 ? false : _d,
    _e = options.coreCount,
    coreCount = _e === void 0 ? 1 : _e,
    _f = options.energyConfig,
    energyConfig = _f === void 0 ? { activeWatts: 20, idleWatts: 5, switchJoules: 0.1 } : _f;
  var logs = [];
  var stepLogs = [];
  ...
  while (completedCount < totalProcesses) {
      ...
      if (core.currentTime <= systemTime) {
        ...
        if (core.currentProcessPid && core.currentProcessPid !== 'CS') {
          ...
          readyQueue.sort(function (a, b) {
            var _a, _b;
            var pA = (_a = a.priority) !== null && _a !== void 0 ? _a : Number.MAX_SAFE_INTEGER;
            var pB = (_b = b.priority) !== null && _b !== void 0 ? _b : Number.MAX_SAFE_INTEGER;
            if (pA !== pB) return pA - pB;
            return a.arrival - b.arrival;
          });
          var currentPrio =
            (_a = current.priority) !== null && _a !== void 0 ? _a : Number.MAX_SAFE_INTEGER;
          if (
            readyQueue.length > 0 &&
            ((_b = readyQueue[0].priority) !== null && _b !== void 0
              ? _b
              : Number.MAX_SAFE_INTEGER) < currentPrio
          ) {
            logDecision(
              systemTime,
              core.id,
              'Preempting '.concat(current.pid, ' for ').concat(readyQueue[0].pid),
              'New process has higher priority ('
                .concat(readyQueue[0].priority, ' < ')
                .concat(currentPrio, ')'),
    ...
    if (duration > 0) {
      ...
      for (var _g = 0, cores_2 = cores; _g < cores_2.length; _g++) {
        var core = cores_2[_g];
        _loop_2(core);
      }
