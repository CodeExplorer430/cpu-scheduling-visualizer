...
exports.runPriority = runPriority;
var utils_js_1 = require('./utils.js');
function runPriority(inputProcesses, options) {
  if (options === void 0) {
    options = {};
  }
      ...
      if (core.currentTime <= systemTime && readyQueue.length > 0) {
        ...
        var queueState = __spreadArray(
          ...
          readyQueue.map(function (p) {
            return ''.concat(p.pid, '(').concat(p.priority, ')');
          }),
          true
        );
        logDecision(
          ...
          'Selected '
            .concat(currentProcess.pid, ' because it has the highest priority (')
            .concat(currentProcess.priority, ').'),
          queueState
        );
        if (
