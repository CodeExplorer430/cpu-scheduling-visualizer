...
interface EDFProcess extends Process {
  remaining: number;
  effectiveDeadline: number;
}

export function runEDF(
  inputProcesses: Process[],
  options: SimulationOptions = {}
): SimulationResult {
  const { coreCount = 1 } = options;

  const processes: EDFProcess[] = inputProcesses.map((p) => ({
    ...p,
    remaining: p.burst,
    effectiveDeadline: p.deadline !== undefined ? p.deadline : p.arrival + p.burst,
  }));
  ...
  while (completed < processes.length) {
    const ready = processes
      ...
      .sort((a, b) => {
        if (a.effectiveDeadline !== b.effectiveDeadline) {
          return a.effectiveDeadline - b.effectiveDeadline;
        }
