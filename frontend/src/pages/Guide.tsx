      ...
      {activeTab === 'algorithms' && (
        <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <section className="space-y-6">
            ...
            <p className="text-gray-600 dark:text-gray-400 text-lg">{t('guidePage.algo.intro')}</p>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {algorithms.map((algo) => (
                <Card
                  key={algo}
                  className="hover:border-indigo-500 dark:hover:border-indigo-400 border border-transparent transition-all"
                >
                  <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2">
                    {t(`guidePage.algo.${algo}.title`)}
                  </h3>
                  <p className="text-sm text-gray-600 dark:text-gray-400">
                    {t(`guidePage.algo.${algo}.desc`)}
                  </p>
                </Card>
              ))}
            </div>
