      ...
      <div className="text-right">
        ...
        <div className="flex space-x-1 mt-1">
          {snapshot?.readyQueue.map(pid => (
            <span key={pid} className="bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 text-xs font-bold px-2 py-1 rounded border border-blue-200 dark:border-blue-800">
              {pid}
            </span>
          ))}
          {(!snapshot?.readyQueue.length) && <span className="text-gray-300 dark:text-gray-600 text-sm italic">Empty</span>}
        </div>
      </div>
    </div>
  );
